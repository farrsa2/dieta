...
(() => {
  ...
  function categoryForProduct(name) {
    ...
    if (/\bfruta\b/.test(n)) return 'FRUTA';

    if (/(jamon serrano|jamon york|fiambre|lomo embuchado)/.test(n)) return 'FIAMBRE';

    if (/(muslo de pavo|pechuga de pavo|pavo fresco|pechuga de pollo|pollo fresco|lomo fresco)/.test(n)) return 'CARNE';
